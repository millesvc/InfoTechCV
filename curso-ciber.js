...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  const progreso = clampPercent(session.data.progreso?.ciber);
  const text = document.getElementById("textoProgresoCiber");
  const bar = document.getElementById("barraCursoCiber");
  if (text) text.textContent = `${progreso}%`;
  if (bar) bar.style.width = `${progreso}%`;
});

async function guardarProgresoCiber(valor) {
  ...
  try {
    await updateNestedProgress(correo, "progreso.ciber", nuevoValor);
    const text = document.getElementById("textoProgresoCiber");
    const bar = document.getElementById("barraCursoCiber");
    if (text) text.textContent = `${nuevoValor}%`;
    if (bar) bar.style.width = `${nuevoValor}%`;
  } catch (error) {
    console.error("Error guardando progreso ciber:", error);
  }
}
