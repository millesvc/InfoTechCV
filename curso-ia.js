...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  const progreso = clampPercent(session.data.progreso?.ia);
  const text = document.getElementById("textoProgresoIA");
  const bar = document.getElementById("barraCursoIA");
  if (text) text.textContent = `${progreso}%`;
  if (bar) bar.style.width = `${progreso}%`;
});

async function guardarProgresoIA(valor) {
  ...
  try {
    await updateNestedProgress(correo, "progreso.ia", nuevoValor);
    const text = document.getElementById("textoProgresoIA");
    const bar = document.getElementById("barraCursoIA");
    if (text) text.textContent = `${nuevoValor}%`;
    if (bar) bar.style.width = `${nuevoValor}%`;
  } catch (error) {
    console.error("Error guardando progreso IA:", error);
  }
}
